...
import 'package:admin/core/neo_secure_storage/neo_secure_storage.dart';
...
class NeoAvatarBloc extends Bloc<NeoAvatarEvent, NeoAvatarState> {
  ...
  _onInitialize(NeoAvatarEventInitialize event, Emitter<NeoAvatarState> emit) {
    _neoWidgetEventSubscription = getIt.get<NeoWidgetEventBus>().listenEvents(
      eventIds: [
        ...
        NeoWidgetEventKeys.neoAvatarChangePhotoEventKey.name,
        NeoWidgetEventKeys.neoAvatarDeletePhotoEventKey.name,
      ],
      onEventReceived: (event) {
        ...
        } else if (NeoWidgetEventKeys.neoAvatarChangePhotoEventKey.name == event.eventId) {
          final imagePath = (event.data ?? '') as String;
          add(NeoAvatarEventSaveAvatarImage(imagePath));
        } else if (NeoWidgetEventKeys.neoAvatarDeletePhotoEventKey.name == event.eventId) {
          add(const NeoAvatarEventDeleteAvatarImage());
        }
      },
    );
  }
  ...
  _onChangeTextStyles(NeoAvatarEventChangeTextStyles event, Emitter<NeoAvatarState> emit) {
    ...
    emit(NeoAvatarState(subTitleStyle: _subTitleStyle, labelTitleStyle: _labelTitleStyle));
  }
